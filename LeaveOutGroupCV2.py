...
class LeaveOutGroupCV2(LeaveOutPercentCV):
    ...
    def __init__(self, 
        ...
        LO_group2 ='Temperature',#'flux_n_cm2_sec',#'fluence_n_cm2',
        *args, **kwargs):
        """
    ...
    def give_me_dic(self):
        ...
        for cvtest in range(0, self.num_cvtests):
            for key in ['rmse']:
                statlist.append(self.cvtest_dict[cvtest][key])
        return statlist
    ...
    def plot(self):
        ...
        df = pd.DataFrame.from_dict(self.cvtest_dict, orient="index")
        #df = df.head(5)
        df.to_csv('./LOGtemp.csv')
    ...
    def set_up_cv(self):
        ...
        for train, test in self.cvmodel.split(self.testing_dataset.input_data,
                        ...
                        d_group):
            group = totall.index.tolist()[cvtest]
